...
{
    registry.view<InputComponent>().each(
        [&](InputComponent &input)
        {
            ...
            if (InputManager::wasKeyReleased(SDL_SCANCODE_UP) ||
                InputManager::wasKeyReleased(SDL_SCANCODE_DOWN))
            {
                input.movement_input.y = 0;
            }

            if (InputManager::wasKeyReleased(SDL_SCANCODE_LEFT) ||
                InputManager::wasKeyReleased(SDL_SCANCODE_RIGHT))
            {
